...
Completed accounts become discoverable only after a separate versioned account-directory opt-in, only while Active, and only to mutually eligible accounts in the exact same normalized self-entered region. The opt-in defaults absent, can be withdrawn from Profile, and is not bundled with service data use or research. Public profile fields cross the account-store boundary for matching; private preferences are evaluated server-side and candidate-side traces remain redacted. Interested remains private unless reciprocal. A mutual decision creates the same deterministic pair connection in both isolated stores, and each message is copied to both participants' stores. Unmatch, block, and polite close attempt reciprocal closure. These cross-store writes are not atomic: a process or disk failure between writes can leave temporary disagreement, and there is no repair queue. This is suitable only for explicit non-production flow testing. A production design needs one transactional relationship/message system or an idempotent durable delivery protocol, plus a deletion/moderation retention policy.
