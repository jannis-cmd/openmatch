...
Implemented API capabilities include editable profile/preferences, finite introductions, mutual connections, text-only messages, unmatch, block, structured reports, opt-in expiring local demo sessions, and a minimal account-isolation foundation with active-session revocation and synchronous account deletion. Native account sessions restore from device-only Expo SecureStore. The account system is intentionally incomplete: verified email/passkeys, recovery, production migration tooling and database authorization, moderation staffing/tools, encryption key management, independent security review, and deployment remain future milestones.
