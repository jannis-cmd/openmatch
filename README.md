...
Release metadata is deliberately narrow: mobile builds request no notification, vibration, overlay, local-network, or external-storage capability; the resolved Android manifest retains only internet access, and iOS rejects arbitrary/local transport exceptions. The web app includes installable raster icons but no service worker that could cache private API responses.
