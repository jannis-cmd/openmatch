...
| Require a user-declared 30-day account-matching availability window | Prototype hypothesis | Apparently inactive profiles are a recurring but weakly measured user complaint; automatic last-active tracking would create behavioral surveillance and ambiguous social pressure | Participatory research may change or remove the duration; prospective testing must examine stale-profile experience, missed opportunity, accessibility, and unequal renewal burden. Thirty days is not claimed as an evidence-backed optimum |
