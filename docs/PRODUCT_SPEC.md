...
The prototype currently saves setup through several individually idempotent requests and treats the final onboarding receipt as the only completion signal. Web and native clients serialize submission, retain the entered form and checked acknowledgements after failure, explain that an earlier step may already be confirmed, and safely replay the same setup on retry. They never describe this multi-request sequence as atomic. A production pilot should replace it with one transactional, versioned setup command so profile, preferences, required consent, optional directory participation, and completion either commit together or not at all.
