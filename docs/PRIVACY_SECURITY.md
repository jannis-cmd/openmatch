...
Any authenticated request that receives `401 session_required` clears the client's raw account token. Web immediately unmounts all private application state and returns to the public entry screen; iOS and Android remove the SecureStore value, clear the in-memory account mode, and show a neutral “session ended” sign-in notice. A revoked or expired token never falls back into the shared demo identity.
