...
Finish the account lifecycle: verified contact ownership, privacy-preserving recovery, production database migrations, endpoint-specific abuse controls, and independent security review. Validate the deliberately minimal active-session design in threat modeling and accessibility testing. Then replace static demo candidates with consented account-to-account eligibility in a non-production test environment. Do not add photos, notifications, or broader matching fields until privacy, inclusion, and safety requirements for those features are resolved.
