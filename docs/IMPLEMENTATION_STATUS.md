...
| Personal-data transparency | JSON export, pause, hide, resume, and synchronous deletion/reset from Profile on web and mobile; deletion returns a timestamped completion receipt only after every local collection and consent state is cleared, while the receipt itself is not retained server-side |
