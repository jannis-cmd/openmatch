...
| Minimal isolated accounts | Web and production mobile clients can create an email/passphrase account, sign in, revoke the current session, and synchronously delete credentials, every session, and isolated application data. Email is normalized, passphrases are stored only as per-account salted scrypt hashes, raw 256-bit session tokens are returned once while only SHA-256 hashes persist, authentication endpoints have a separate conservative throttle, and every protected operation is routed to an account-specific SQLite store. Tests prove two accounts cannot see each other's profile or connections. Email verification, recovery, durable secure native token storage, migration tooling, and security review remain required before a pilot |
...
Finish the account lifecycle: verified contact ownership, privacy-preserving recovery, durable native token storage, session/device management, database migrations, endpoint-specific abuse controls, and independent security review. Then replace static demo candidates with consented account-to-account eligibility in a non-production test environment. Do not add photos, notifications, or broader matching fields until privacy, inclusion, and safety requirements for those features are resolved.
