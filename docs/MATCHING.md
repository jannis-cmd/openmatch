...
The API records one observation only when a person explicitly chooses Interested or Pass. It stores the visible A-side compatibility vector and the recorded selection probability, never the other person's private weights. `/v1/preferences/suggestions` returns the observation count, published minimum, confidence, caveat, and `automaticChanges: false`; accepting a suggestion uses the ordinary explicit preference-update path. A person can clear all learning examples independently on web, iOS, or Android without rewriting the Interested/Pass decisions that control introductions and connections.
