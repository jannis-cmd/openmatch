...
- A primary sign-in email change requires the active session, the current passphrase, and separate single-use codes from both the current confirmed inbox and proposed inbox. The old address remains authoritative until one atomic completion, which revokes every other session and attempts sparse notices to both addresses. This is the stricter OWASP baseline for password-only accounts, not MFA or identity proofing.
