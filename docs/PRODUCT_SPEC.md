...
- Export, pause, hide, and delete controls available from Profile without contacting support. Permanent credential deletion requires fresh current-passphrase verification in addition to the active session, following current OWASP sensitive-feature guidance and NIST SP 800-63-4's reauthentication model. Export, reset, and permanent account deletion are mutually serialized; a rejected request stays in context and explicitly says that no download or deletion completed, while the UI changes only after the corresponding server result or deletion receipt.
- A prominent public `/delete-account` resource links directly into web sign-in and then opens Profile's permanent deletion control. Local account deletion erases profile-linked decisions, connections, and delivered messages from every other prototype account store before reporting completion; this is idempotent local cleanup, not yet a production distributed-erasure operation.
- A created account can be deleted from web, iOS, or Android before profile setup or prototype consent is completed. Deletion never requires someone to provide additional dating data or join matching first.
...
- Passwords accept 15–128 characters without composition or periodic-change rules, allow password-manager autofill/paste, require the current password before an in-account change, and replace all other sessions after a successful change.
- Forgotten passwords use the familiar email-link flow in self-hosted web, iOS, and Android. The request never reveals whether an account exists; a valid short-lived link sets a new password and replaces all prior WhyMatch sessions. The older offline-code mechanism remains available only to standalone regression tests and is not part of the product UI.
...
- A primary sign-in email change requires the active session, the current passphrase, and separate single-use codes from both the current confirmed inbox and proposed inbox. The old address remains authoritative until one atomic completion, which revokes every other session and attempts sparse notices to both addresses. A signed-in person can cancel the pending change, invalidating both codes while leaving the current address unchanged. This is the stricter OWASP baseline for password-only accounts, not MFA or identity proofing.

The first-run product setup is intentionally short: public name, adult age, short biography, one or more structured gender identities with an optional self-description, overlapping discovery groups, mutual gender/age/distance boundaries, and four named priority levels (Off, Low, Medium, High). Because values, smoking/children compatibility, and schedule are active prototype factors, the executable clients use stable presets for those fields and for the prompt question; only the biography, prompt answer, optional pronouns, and optional gender self-description remain free text. No demo answer is silently treated as the user’s own. Gender is never inferred, multi-select and self-description prevent a binary/“other” model, and partner preferences remain private. It does not use a personality test or claim that questionnaire length improves relationship outcomes. Inclusion, consent, and safety details required for a real pilot must continue through focused participatory research rather than inference from this executable baseline.
...
Required in the executable prototype: display name/pseudonym, age, approximate region, structured gender selection, people sought, relationship intention, short biography, and availability/readiness. One optional profile photo can be selected from the device library, compressed locally, replaced, or removed. Camera capture, image analysis, beauty scoring, face recognition, and photo-based matching remain prohibited; production media storage, moderation, consent, accessibility, and outing-risk controls remain pilot blockers.
...
- Each executable card shows the optional user-selected photo, core information, biography, intention, shared/compatible factors, known differences, and an exact score explanation. Photo presence and content never affect eligibility or score. Consent, secure production media processing, abuse operations, accessibility, outing risk, and inclusion remain unresolved for a real-person pilot; image analysis and beauty scoring are prohibited.
