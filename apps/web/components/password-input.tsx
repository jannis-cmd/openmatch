...
export function PasswordInput({
  ...
  showLabel = "Show",
  hideLabel = "Hide",
  tooLongMessage = "Use 128 characters or fewer.",
  maxLength,
  ...props
}: PasswordInputProps) {
  ...
  const valueLength = String(props.value ?? props.defaultValue ?? "").length;
  const tooLong = typeof maxLength === "number" && valueLength > maxLength;
  ...
  return (
    <span className="password-field">
      ...
      {tooLong && !props["aria-describedby"] && (
        <span className="password-limit" role="alert">
          {tooLongMessage} ({valueLength}/{maxLength})
        </span>
      )}
    </span>
  );
